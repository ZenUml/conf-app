...
description: >
  ...
  publish, and verify rendered spec viewer in the Forge iframe. Invoked by release-app Step 2.6 when
...
2. **Infer from conversation** — Release tag, `/release-app` variant, Step 2.6 pass-through, or user/PR wording → test **that** product only when unspecified.
