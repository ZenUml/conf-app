...
description: >-
  ...
  (total revenue, renewals due in a window, overdue payers, top payers) use the `marketplace`
  skill instead. Discriminator: one domain in the question -> this skill; a cohort/time-window/ranking
  -> marketplace. Triggers on tenant, whois, "is <domain> paying", paid status, tenant lookup,
