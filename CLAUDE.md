...
**Remote storage declaration evidence, 2026-08-10:** PR #383 stopped stripping `operations: storage` and `storage.inScopeEUD: true` from the Full, Diagramly, and AsyncAPI manifests. Before the change those fields were absent; they were not explicitly `inScopeEUD: false`. Forge classified every resulting staging deployment as a **minor** update: Full `3.447.0` → `3.448.0`, Diagramly `2.636.0` → `2.637.0`, and AsyncAPI `5.126.0` → `5.127.0`; each staging installation auto-updated to `Latest` without an approval wall. Do not describe PR #383 itself as a major version or an admin re-consent change. The Forge remotes reference's major-version row is specifically for an explicit `inScopeEUD: false` → `true` transition. Production deploy output remains the final check in case a release bundles some other major-version change.
