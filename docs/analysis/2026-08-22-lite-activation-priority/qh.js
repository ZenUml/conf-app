...
function main(){
  return Events({from_date:'2026-07-23', to_date:'2026-08-22', event_selectors:[{event:'macro_viewed'}]})
  .filter(function(e){ return !isInternal(e.properties.client_domain) && okUser(e) && e.properties.product_type==='lite' && e.properties.surface==='viewer'; })
  .groupBy([function(e){return String(e.properties.macro_type);}, 'distinct_id'], mixpanel.reducer.count())
  .groupBy([function(r){return r.key[0];}], [mixpanel.reducer.count(), mixpanel.reducer.sum(function(r){return r.value;})]);
}
