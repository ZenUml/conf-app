...
6. **[Phase 5a] Space-admin activity measurement** ✅ *shipped; **read back 2026-07-28**.* `space_admin_active` probe on the page-banner load (`src/utils/paywall/spaceAdminProbe.ts`): Lite-only, throttled once/30d per `domain:space`, fires only when the current user is a space admin.
   **Go/no-go answer — decisive GO on reach.** 60d (2026-05-29 → 07-28), Mixpanel 3373228:
   - **24,388** unique space admins across **709** external Lite tenants (65,680 events).
   - Within the 19 CSS-enrolled tenants: **5,021** unique admins — against **358** unique users the banner's author-only gate actually reached, and **642** who hit the paywall. **4,815 admins had never seen anything.**
   - Caveat on quality: Confluence counts anyone who created a space (incl. personal spaces) as a space admin, so this is a reliable *reachable-population* figure and an **unverified** *buyer* figure.
   **But reach is only two-thirds of the problem:** 206 of the 642 paywall-hitters (**32%**) were already space admins and still converted zero times. Split by tenant this is two distinct failure modes — `airwallex` 83% admin overlap (buyer present, won't buy → **offer** problem) vs `vin3s` 15% (buyer absent → **reach** problem). 5b addresses reach only; do not expect it to move an offer-problem tenant.
