...
**Last updated:** 2026-09-03 · **Variant scope:** Lite only (Full/Diagramly bypass via `isLite() === false`) · **Day-to-day monitoring:** `.claude/skills/paywall/SKILL.md`
...
7c. **Pricing survey in the paywall modal** (*built, pending release*). Answers the same question the email feedback-offer (`extend-space-license` skill) has been asking manually since 2026-07-25 with 0 replies in 8+ asks: what would this space actually pay. Placed in the modal **after "Request extension" and before the JSM deep link**, so a customer who would otherwise leave the product for a support ticket can answer in place instead. Four questions: (1) role, space admin, diagram author, site app admin, or other; (2) a 4-number price-sensitivity battery (too cheap / bargain / getting expensive / too expensive, in USD/year to unlock the space); (3) preferred vs. least-preferred billing model (per space/year, per Confluence user/month, per active diagram author, per number of diagrams); (4) the internal blocker to a permanent upgrade (budget, admin approval, procurement, nobody owns it, or other). Submitting grants **15 days, user-scoped** automatically (`license:<cloudId>:<spaceKey>:<accountId>` in `SPACE_LICENSE_KV`, `activatedBy: survey:pricing-15d:<responseId>`), one grant per user per space ever, with no ticket and no wait on a reply. Each answer is saved to D1 as it's given rather than only on final submit, so an abandoned survey is still visible to us, not lost. Events: `paywall_survey_shown`, `paywall_survey_answered` (per question), `paywall_survey_submitted`, `paywall_survey_skipped`. Answers land in D1 table `PaywallSurveyResponse` (`functions/migrations/0024_add_paywall_survey_response.sql`). The email offer was cut from 60 days to the same 15 so neither path out-rewards the other.
