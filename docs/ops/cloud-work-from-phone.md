...
```
 ...
 │ codexloom :4870, seedmux, Codex   │      │ connectors (work): Mixpanel, Gmail,     │
 │ desktop scheduled tasks (none)    │      │  Slack, Figma, HubSpot, Google Drive    │
...
every REST path in the table below is laptop-only; only the **connectors** and **GitHub** work.
...
| Release (release-app skill) | nothing local — GitHub Actions | works, but policy: no release without PVT + spot check |
...
2. Re-provision the "Conf App" cloud environment (claude.ai → Code → composer environment picker → Cloud → "Conf App" → settings). Only you can do this: the values are secrets and the editor is your account. Per row, **Add credential** with the host and the token so the proxy attaches it and the host becomes reachable (the editor says credentials also open the host even under Trusted):
   - `*.atlassian.net` and `marketplace.atlassian.com` → `FORGE_EMAIL` / `FORGE_API_TOKEN` from `.env.forge.local` (Confluence REST v2, Marketplace export, `forge` CLI).
   - `zenuml.atlassian.net` → `JSM_EMAIL` / `JSM_API_TOKEN` (service desk; the Forge token only 403s there).
   - `api.cloudflare.com` → `CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_API_TOKEN` (D1, KV, Pages).
   - `mixpanel.com` → `MIXPANEL_SA_USER` / `MIXPANEL_SA_SECRET` (JQL; the connector already covers reports).
   - `ATLASSIAN_USERNAME` / `ATLASSIAN_PASSWORD` / `ATLASSIAN_OTP` (robot, `tests/e2e-tests/.env`) only if headless Playwright smoke tests should run in the cloud.
   Alternatively set **Network access = Full** (options: None / Trusted / Full / Custom) and put the same pairs in Environment variables; the editor warns those are visible to anyone using the environment, so prefer API credentials.
   Add a setup script: `pnpm install --frozen-lockfile` (gives `forge` and `wrangler`).
...
- **Works today, no setup:** Mixpanel questions, health checks, session-replay reads, dashboard creation through the Mixpanel connector (`mixpanel`, `health-check`, `conf-app` skills); Gmail triage and reply drafts (sending needs your go-ahead per message); Figma design reads; Slack; HubSpot.
...
- Routines: daily/weekly watches already exist on this environment (paywall re-read, docat weekly). New one-shot reminders via `RemoteTrigger`.
