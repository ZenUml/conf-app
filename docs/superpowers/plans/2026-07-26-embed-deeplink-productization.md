...
- **Open verification item before the Diagramly release:** a tenant with both Lite and Diagramly installed has two apps matching `conf-lite.zenuml.com/d/*/*`; confirm Confluence's paste behavior with two matching macros, and whether Diagramly's embed viewer can resolve Lite-minted custom content (app-namespaced content types) — must not fail into a converts-but-renders-nothing state.
