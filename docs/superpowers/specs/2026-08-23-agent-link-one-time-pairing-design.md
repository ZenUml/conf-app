...
- The initialize response issues a random `Mcp-Session-Id` response header and stores a safe product label for that initialized transport. Supported protocol revisions are echoed; unknown revisions receive the latest supported revision, following [MCP lifecycle negotiation](https://modelcontextprotocol.io/specification/2025-11-25/basic/lifecycle). Unsupported subsequent protocol headers return 400; terminated transport sessions return 404, following [Streamable HTTP session management](https://modelcontextprotocol.io/specification/2025-11-25/basic/transports).
...
- A second `connect` on the same MCP session may replace its active target. An invalid new code leaves the existing pairing intact. After a valid new binding is committed, the previous target claim is released. Concurrent replacements compare the prior binding so a losing request rolls back its claim.
- `DELETE /agent-link/mcp` terminates the MCP transport session and releases its target claim when clients send the standard termination request. Transport termination atomically removes its mapping and prevents any in-flight claim from committing. Target TTL remains the crash-recovery backstop.
...
Macro reconnect first confirms HTTP revocation and content-lock release, then mints a new code. Closed and expired codes retain a terminal record across Durable Object eviction, with stored diagram bodies removed.
...
`session_created` without `pairing_completed` is a missing outcome, not proof of abandonment. `session_expired(had_agent_connected=false)` is an observed unpaired expiry. Invalid unknown-code failures remain unobservable to the Macro; preserve that uncertainty.
...
9. The Bearer auth requirement and its tests are replaced; this change explicitly replaces the per-session Bearer contract rather than keeping a compatibility lane.
...
This change replaces the per-session Bearer setup UX with one-time pairing; it does not infer current production exposure. Deployment order remains companion Worker first, then Pages/frontend, because the Pages MCP handler depends on the new Durable Object endpoints.
