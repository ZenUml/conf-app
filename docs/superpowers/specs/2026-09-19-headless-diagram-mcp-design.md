...
Completed 2026-09-25, per [ADR 0008](../../adr/0008-agent-link-runs-its-own-authorization-server.md): our authorization server — RFC 8414 metadata and the RFC 9728 resource document (both served from the root middleware, because the Pages router ignores dot-directories, so `functions/.well-known/` is never routed), Dynamic Client Registration, PKCE (S256 only), opaque token issuance with rotating refresh, and the per-(user, client) consent screen §5.1 requires. Verified end to end against real Atlassian on 2026-09-25: register → authorize → Atlassian consent → our consent → code → token, with the code single-use and the consent recorded. Of the human steps, the Atlassian OAuth app is registered (2026-09-19: developer-console app `a7f2a21d-a28c-4155-af0f-df365b5b94ef` "ZenUML Agent Link", the four `REQUIRED_SCOPES`, callbacks on `zenapi.zenuml.com`, `conf-stg-lite.zenuml.com` and `localhost:8080` at `/agent-link/oauth/callback`, distribution private) and its client id is committed as the `ATLASSIAN_OAUTH_CLIENT_ID` var, read by `functions/agent-link/oauth/appConfig.ts`. Provisioned 2026-09-25: the `OAUTH_GRANT_KV` namespaces (`agent-link-oauth-grants-stg` / `-prod`, ids in the wrangler configs) and the `ATLASSIAN_OAUTH_CLIENT_SECRET` and `OAUTH_GRANT_SECRET` Pages secrets on `conf-stg-lite` and `conf-lite`.
