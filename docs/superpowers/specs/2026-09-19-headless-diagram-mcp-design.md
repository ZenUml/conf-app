...
Still open, and blocked on §12.6: our authorization server — RFC 8414 metadata, DCR, PKCE, our own token issuance and validation, and the consent screen §5.1 requires. Of the human steps, the Atlassian OAuth app is registered (2026-09-19: developer-console app `a7f2a21d-a28c-4155-af0f-df365b5b94ef` "ZenUML Agent Link", the four `REQUIRED_SCOPES`, callbacks on `zenapi.zenuml.com`, `conf-stg-lite.zenuml.com` and `localhost:8080` at `/agent-link/oauth/callback`, distribution private) and its client id is committed as the `ATLASSIAN_OAUTH_CLIENT_ID` var, read by `functions/agent-link/oauth/appConfig.ts`. Still to provision, needing Cloudflare access: the `OAUTH_GRANT_KV` namespaces (placeholder ids in the wrangler configs), and the `ATLASSIAN_OAUTH_CLIENT_SECRET` and `OAUTH_GRANT_SECRET` Pages secrets.
