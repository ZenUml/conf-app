...
function rpc(method: string, params?: unknown, id: number | string = 1) {
  if (method === 'initialize' && params === undefined) params = { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'test-client', version: '1' } };
...
function makeRequest(
  ...
  if (opts.mcpSessionId) {
    headers['Mcp-Session-Id'] = opts.mcpSessionId;
    // These unit fixtures represent an already initialized transport. The
    // real initialization/pairing sequence is covered in mcpPairing.spec.ts.
    mcpBindingRegistry.rememberClient(opts.mcpSessionId, 'an AI agent');
  }
...
describe('POST /agent-link/mcp', () => {
  ...
  it('DELETE releases the local MCP-session binding and one-time claim', async () => {
    ...
    const afterDelete = await onRequestPost({ request: new Request('https://example.com/agent-link/mcp', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'Mcp-Session-Id': firstSessionId },
      body: JSON.stringify(rpc('tools/call', { name: 'read_page', arguments: {} })),
    }) } as any);
  ...
  it('DELETE reports unavailability when transport termination cannot be confirmed', async () => {
...
function makeDoEnv(responses: FakeDoResponses) {
  const stub = {
    fetch: async (url: string, init?: RequestInit) => {
      ...
      if (path.endsWith('/mcp-transport')) return new Response(JSON.stringify({ clientName: 'an AI agent', expiresAtMs: Date.now() + 86_400_000 }), { headers: { 'Content-Type': 'application/json' } });
...
describe('POST /agent-link/mcp with an AGENT_LINK Durable Object binding', () => {
      ...
      AGENT_LINK: {
        ...
        get: (id: { name: string }) => ({
          fetch: async (url: string, init?: RequestInit) => {
            ...
            if (path === '/mcp-transport') return new Response(JSON.stringify({ clientName: 'an AI agent', expiresAtMs: Date.now() + 86_400_000 }), { headers: { 'Content-Type': 'application/json' } });
  ...
  describe('sliding-TTL bump + guardrail reporting', () => {
    ...
    it('static resources/read is public and does not bump the target session', async () => {
      ...
      expect(fetchedUrls.filter(url => new URL(url).pathname !== '/mcp-transport')).toEqual([]);
    });
        ...
        AGENT_LINK: {
          ...
          get: (id: { name: string }) => {
            if (id.name.startsWith('mcp:test:')) {
              ...
              return {
                fetch: async () =>
                  new Response(JSON.stringify({ token, clientName: 'an AI agent', expiresAtMs: Date.now() + 600_000 }), {
                    status: 200,
                    headers: { 'Content-Type': 'application/json' },
                  }),
              };
