...
export class McpBindingRegistry {
  private readonly clients = new Map<string, { label: string; expiresAtMs: number }>();

  rememberClient(session: string, label: string): void { this.clients.set(session, { label, expiresAtMs: Date.now() + 24 * 60 * 60 * 1000 }); }
  getClient(session: string): string | undefined {
    const client = this.clients.get(session);
    if (!client || client.expiresAtMs <= Date.now()) { this.clients.delete(session); return undefined; }
    return client.label;
  }
  forgetClient(session: string): void { this.clients.delete(session); }
