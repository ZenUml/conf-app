// PlantUML DSL guidance surfaced to the connected agent when the bound diagram
// is a PlantUML macro, so an MCP client writes valid PlantUML on the first try
// instead of mixing diagram sub-types, misusing !define, duplicating the
// @startuml/@enduml envelope, or leaking Mermaid/ZenUML syntax (all real,
// ranked failure signatures). Served the same three ways as the ZenUML guide
// (mcp.ts + mcpTools.ts) and mirrored to guides/plantuml.md for Track E1.
//
// DATA-DRIVEN: rules ranked by sanitized AI-Repair failure signatures from the
// diagram-modification corpus (22 scored PlantUML rows) plus the generation-
// failure population. See evidence/B-signature-mining.md §3. PlantUML's native
// error message is near-uninformative ("Assumed diagram type: X"), so every
// signature was derived by reading the input→output diff, not the error text.
// Identifiers in the examples are genericised for client privacy; the
// syntactic shape of each bug is preserved.
//
// PORTED FROM THE OFFLINE EVAL (diagramly.ai-agent-link-eval, Track E1),
// guides-r1/plantuml-v4.md — the deep-fix winner (69.0% -> 72.4% clean-fix,
// paired judge-v2; see that repo's reports/E1-ROUND23-SYNTHESIS.md +
// reports/E1-PLANTUML-DEEPFIX.md). What changed vs. the previous version of
// this file:
//   - The "Minimal-edit contract" section (same lever as mermaid's, ported
//     alongside it).
//   - Rule #2 REWRITTEN, not just reworded: the old advice ("hallucinated
//     SET_SOMETHING(...) call -> !define SOMETHING value") was verified
//     against the real C4-PlantUML stdlib source and a rendered comparison to
//     be syntactically valid but semantically INERT — the C4-PlantUML macro
//     library never reads a plain !define'd constant for element styling, so
//     that "fix" cleared the parse error while silently dropping the styling
//     intent. Replaced with the real mechanism: `UpdateElementStyle()` /
//     `AddElementTag()` + `$tags=`.
//   - The "This is PlantUML" comparison table's arrow row was WRONG and is
//     now corrected: `->>` / `-->>` are real, native PlantUML arrow tokens
//     (async/thin-arrowhead variants), not a Mermaid-ism to rewrite as
//     `->`/`-->`. The old (wrong) row was the traced root cause of an agent
//     "correcting" every legitimate async arrow in a diagram instead of
//     applying rule #8's actual fix.
//   - Rule #8 (declared alias vs. how it's referenced) added, unchanged since
//     first introduced — it was never actually exercised by the model until
//     the arrow-table row above was fixed.

/** Full PlantUML DSL reference — used for `initialize.instructions` + the resource. */
export const PLANTUML_DSL_GUIDE = `# Writing PlantUML DSL for update_diagram
...
export const PLANTUML_DSL_TOOL_HINT =
  'This "plantuml" diagram uses PlantUML DSL (NOT Mermaid/ZenUML): wrap in ' +
  'exactly one `@startuml`…`@enduml` pair; don\'t mix two sub-types (e.g. ' +
  'activity `partition`/`start` with sequence `participant`/arrows) or paste a ' +
  'Mermaid header; declare actors as `actor "X" as Y` (no `@` prefix); use ' +
  'straight ASCII quotes; `->>`/`-->>` are real PlantUML async arrows, not a ' +
  'Mermaid-ism to "fix". Full guide: the `plantuml://dsl-guide` resource.';

/** URI of the guide exposed via resources/list + resources/read. */
export const PLANTUML_DSL_GUIDE_URI = 'plantuml://dsl-guide';
