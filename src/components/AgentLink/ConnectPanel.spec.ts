...
describe('ConnectPanel', () => {
  ...
  it('a remembered client cue retains the visible pairing prompt and connector setup', () => {
    rememberAgentLinkClient('Codex')
    const wrapper = mountPanel({ state: 'waiting', token: 'fresh-code' })
    expect(wrapper.get('[data-testid="agent-link-remembered-client"]').text()).toContain('Codex')
    expect(wrapper.get('[data-testid="agent-link-prompt"]').text()).toContain('fresh-code')
    expect(wrapper.get('[data-testid="agent-link-setup-disclosure"]').attributes('open')).toBeDefined()
  })

  it.each(['recovery_exhausted', 'incompatible'] as const)('%s offers a fresh session without exposing the old pairing code', async (state) => {
    const wrapper = mountPanel({ state, token: 'old-code' })
    expect(wrapper.find('[data-testid="agent-link-prompt"]').exists()).toBe(false)
    expect(wrapper.text()).not.toContain('old-code')
    await wrapper.get('[data-testid="agent-link-reconnect-btn"]').trigger('click')
    expect(wrapper.emitted('reconnect')).toHaveLength(1)
    if (state === 'incompatible') expect(wrapper.text()).toContain('Update')
  })

  it('Codex setup copies the stable MCP endpoint without the pairing code', async () => {
    const wrapper = mountPanel({ state: 'waiting', token: 'private-pairing-code' })
    await wrapper.get('[data-testid="agent-link-setup-codex-tab"]').trigger('click')
    const command = wrapper.get('[data-testid="agent-link-setup-command"]').text()
    expect(command).toMatch(/^codex mcp add conf-agent --url /)
    expect(command).toContain('/agent-link/mcp')
    expect(command).not.toContain('private-pairing-code')
    await wrapper.get('[data-testid="agent-link-copy-setup-btn"]').trigger('click')
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(command)
    await wrapper.get('[data-testid="agent-link-setup-claude-tab"]').trigger('click')
    expect(wrapper.get('[data-testid="agent-link-setup-command"]').text()).toMatch(/^claude mcp add --transport http conf-agent /)
  })
  ...
  it('waiting state shows the stage ladder once the agent is seen', () => {
    ...
    expect(w.find('[data-testid="agent-link-setup"]').exists()).toBe(true) // setup hidden after handshake
  })
