<template>
  <div class="agent-link-panel" :class="`agent-link-panel--${state}`" data-testid="agent-link-panel">
    ...
    <div class="agent-link-panel__scroll">
      <!-- Waiting keeps the one-time code, live TTL and credential-free setup visible. -->
      <template v-if="state === 'waiting'">
        <div data-testid="agent-link-waiting">
          <h3 class="agent-link-panel__heading">Edit with your agent</h3>
          <p v-if="rememberedClient" class="agent-link-panel__hint" data-testid="agent-link-remembered-client">Last connected with {{ rememberedClient.label }}. Paste this new code into your agent to connect again.</p>
          ...
          <details open class="agent-link-panel__disclosure" data-testid="agent-link-setup-disclosure">
...
import { readAgentLinkClientMemory, AGENT_LINK_CLIENT_MEMORY_EVENT, AGENT_LINK_CLIENT_MEMORY_KEY } from '@/composables/agentLink/clientMemory'
...
const rememberedClient = ref(readAgentLinkClientMemory())
function refreshRememberedClient() { rememberedClient.value = readAgentLinkClientMemory() }
function onClientMemoryStorage(event: StorageEvent) {
  if (event.key === null || event.key === AGENT_LINK_CLIENT_MEMORY_KEY) refreshRememberedClient()
}
onMounted(() => {
  window.addEventListener(AGENT_LINK_CLIENT_MEMORY_EVENT, refreshRememberedClient)
  window.addEventListener('storage', onClientMemoryStorage)
})
onBeforeUnmount(() => {
  window.removeEventListener(AGENT_LINK_CLIENT_MEMORY_EVENT, refreshRememberedClient)
  window.removeEventListener('storage', onClientMemoryStorage)
})
...
const setupClient = ref<'Claude Code' | 'Codex'>(rememberedClient.value?.label === 'Codex' ? 'Codex' : 'Claude Code')
const mcpAddCommand = computed(() => setupClient.value === 'Codex'
  ? `codex mcp add conf-agent --url ${agentLinkMcpUrl()}`
  ...
  setup() {
    return () =>
      h('div', { class: 'agent-link-panel__setup', 'data-testid': 'agent-link-setup' }, [
        h('div', { role: 'group', 'aria-label': 'Choose your agent', class: 'agent-link-panel__setup-clients' },
          (['Claude Code', 'Codex'] as const).map((client) => h('button', {
            type: 'button',
            class: 'agent-link-panel__btn',
            'aria-pressed': setupClient.value === client,
            'data-testid': `agent-link-setup-${client === 'Codex' ? 'codex' : 'claude'}-tab`,
            onClick: () => { setupClient.value = client },
          }, client)),
        ),
        h(
          'pre',
...
// Verified presence now means connect(code) succeeded; the composable
// advances to the connected view as soon as that signal arrives.
...
<style scoped>
.agent-link-panel__setup-clients { display: flex; gap: 8px; margin-bottom: 8px; }
.agent-link-panel__setup-clients [aria-pressed="true"] { border-color: #2563eb; color: #2563eb; }
.agent-link-panel {
