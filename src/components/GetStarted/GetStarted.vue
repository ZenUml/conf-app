    ...
    <div class="action-section">
      ...
      <div v-if="result" class="action-result" :class="{ ok: result.ok, err: !result.ok }" role="status">
        ...
        <p v-else-if="result.ok">
          Request accepted. Check this space for the examples page before trying again.
        </p>
        ...
        <template v-else>
          ...
          <p v-if="result.orphanDraftPageId">
            An unfinished draft remains in this space. Check your drafts before trying again.
          </p>
        </template>
      </div>
...
const ANALYTICS_BASE = { feature_area: "confluence", surface: "get_started" };
const FAILURE_MESSAGES = {
  not_authorized: "A Confluence site admin needs to create the examples page.",
  space_not_found: "We could not find that space. Check its space key and your access.",
...
function safeFailureReason(result) {
  return Object.hasOwn(FAILURE_MESSAGES, result?.error) ? result.error : "unexpected";
}
...
// A local deadline bounds the waiting UI; it does not cancel the server run
// or establish failure. Report timeout separately and guide duplicate checks.
...
export default {
  ...
  methods: {
    async createExamplesPage() {
      ...
      const recordResult = (outcome, failureReason) => {
        trackAnalyticsEvent("get_started_examples_result", {
          ...ANALYTICS_BASE,
          action: "create_examples_page",
          examples_result: outcome,
      ...
      this.pendingTimeoutId = timeoutId;
      ...
      try {
        ...
        this.result = res && typeof res.ok === "boolean"
          ? res
          : { ok: false, error: "invalid_response" };
        const outcome = examplesResult(this.result);
        if (outcome === "failed" && this.result.ok) {
          this.result = { ok: false, error: "invalid_response" };
        }
        recordResult(outcome, outcome === "failed" ? safeFailureReason(this.result) : undefined);
        this.busy = false;
      } catch {
        clearTimeout(timeoutId);
        if (requestSeq !== this.requestSeq || timedOutHere) return;
        ...
        recordResult("failed", "invoke_failed");
        this.busy = false;
      }
    },
    ...
    async openResultPage() {
      if (this.resultPageUrl) {
        trackAnalyticsEvent("get_started_action_clicked", { ...ANALYTICS_BASE, action: "open_examples_page" });
