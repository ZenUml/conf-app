...
describe('GenericViewer - Upgrade Features', () => {
  ...
  describe('Continue Editing Button', () => {
    it('emits edit and closes modal when continuing from a blocked Edit click', async () => {
      const wrapper = mount(GenericViewer, {
      ...
      vm.edit()
      expect(vm.showUpgradeModal).toBe(true)
      expect(vm.pendingEditAction).toBe(true)
      expect(eventBusSpy).not.toHaveBeenCalledWith('edit')

      vm.onContinueEditing()

      expect(vm.showUpgradeModal).toBe(false)
      expect(vm.pendingEditAction).toBe(false)
      expect(eventBusSpy).toHaveBeenCalledWith('edit')
    })

    it('does not emit edit when continuing from the upgrade badge', async () => {
      const wrapper = mount(GenericViewer, {
        global: { plugins: [store] }
      })
      await wrapper.vm.$nextTick()
    ...
    it('resets pendingEditAction when the modal is dismissed via close', async () => {
      const wrapper = mount(GenericViewer, {
        global: { plugins: [store] }
      })
      await wrapper.vm.$nextTick()
    ...
    it('wires the continue-editing button click on the rendered router to onContinueEditing', async () => {
      const wrapper = mount(GenericViewer, {
        global: { plugins: [store] }
      })
      await wrapper.vm.$nextTick()
      ...
      // Emit the kebab-case event from the router exactly as the real child does
      const router = wrapper.findComponent({ name: 'UpgradePromptRouter' })
      expect(router.exists()).toBe(true)
      router.vm.$emit('continueEditing')
      await wrapper.vm.$nextTick()
