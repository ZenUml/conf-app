...
import { rememberAgentLinkClient, normalizeAgentLinkClientLabel } from './clientMemory'
...
export interface UseAgentLinkSessionOptions {
  ...
  relay?: {
    ...
    revokeSession?: (token: string, ctx: AgentLinkBoundContext) => Promise<void>
    connect?: (
      wsUrl: string,
...
export interface AgentLinkSessionApi {
  ...
  // rationale. Verified pairing advances the connection state immediately.
...
export function useAgentLinkSession(
  ...
  // verified means connect(code) succeeded and advances the FSM immediately.
  ...
  // Verified pairing advances the session before the first op. Subsequent
  ...
  function handleRelayStateEvent(event: RelayStateEvent): void {
    ...
    if (event.type === 'error' && event.unexpected === true) {
      diagnostic({ diagnostic_origin: 'error', reconnect_attempt: event.reconnectAttempt ?? 0 })
    } else if (event.type === 'reconnecting') {
      diagnostic({ diagnostic_origin: 'reconnect_attempt', reconnect_attempt: event.attempt })
    } else if (event.type === 'close' && event.unexpected === true) {
      diagnostic({
        diagnostic_origin: 'close', reconnect_attempt: event.reconnectAttempt ?? 0,
      ...
      diagnostic({ diagnostic_origin: 'reconnect_exhausted', reconnect_attempt: event.attempt ?? 0, reconnect_outcome: 'exhausted' })
    } else if (event.type === 'open' && (event.reconnectAttempt ?? 0) > 0) {
      diagnostic({ diagnostic_origin: 'reconnect_succeeded', reconnect_attempt: event.reconnectAttempt, reconnect_outcome: 'succeeded' })
    }
    if (['closed', 'expired', 'incompatible', 'recovery_exhausted'].includes(state.value)) return
    if (event.type === 'status' && event.activity?.type === 'protocol_incompatible') {
    ...
    if (event.type === 'status') {
      ...
      if (event.activity?.type === 'agent_presence') {
        ...
        if (stage !== progressStage.value) {
          ...
          if (event.activity.clientName) agentClientName.value = normalizeAgentLinkClientLabel(event.activity.clientName)
          ...
          if (stage === 'verified') {
            rememberAgentLinkClient(agentClientName.value, now())
            onAgentConnected()
    ...
    // Only exhaustion is terminal; an individual error is still retryable.
    ...
    // Socket errors do not prove recovery has stopped. Keep the retry state.
    if (event.type === 'error') {
  ...
  function attemptReattach(): void {
    ...
    if (persisted.state === 'incompatible' || persisted.state === 'recovery_exhausted') {
      hydrateFrom(persisted)
      return
    }
  ...
  function hydrateFrom(session: AgentLinkHandoffSession): void {
    ...
    if (token.value === session.token && !relayClient) {
      if (session.state === 'incompatible' || session.state === 'recovery_exhausted') {
        state.value = session.state
        resetThinking()
      }
    }
