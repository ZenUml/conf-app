...
export default function useCustomerSuccessServiceState() {
  const STORAGE_KEY = "customer_success_service_state";
  const DEFAULT_STATE = { users: {}, lastUpdated: new Date() };
  let account = ref<IUser | null>(null);

  const checkStateOfCustomerSuccessService = async () => {
  ...
  const updateStateOfCustomerSuccessService = async () => {
    account.value = await new ApWrapper2(AP)._getCurrentUser();
    const localState = getLocalState(STORAGE_KEY, DEFAULT_STATE);
  ...
  return {
    checkStateOfCustomerSuccessService,
    updateStateOfCustomerSuccessService,
  };
}
