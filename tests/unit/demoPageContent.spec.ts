...
const originalVariantEnv = vi.hoisted(() => {
  const values = {
    CONNECT_KEY: 'gptdock-confluence', CUSTOM_CONTENT_KEY: 'gpt-custom-content-key',
    APP_LABEL: 'Diagramly for Confluence', SEQUENCE_MACRO_KEY: 'gpt-diagram-macro', LITE_KEY_SUFFIX: '',
  };
...
afterAll(() => {
  for (const [key, value] of Object.entries(originalVariantEnv)) {
...
describe('demoPageContent', () => {
  it('exports the canonical title (uses the configured Diagramly identity)', () => {
