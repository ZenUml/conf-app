...
describe('Forge export resolver (src/export.js)', () => {
  ...
  // The shared exportMacro runs for every variant. The AsyncAPI spec fallback
  // (an extra custom-content fetch in the no-attachment branch) must only run
  // on the asyncapi variant so lite/full/diagramly exports stay fast.
  describe('AsyncAPI spec-export fallback gating (PRODUCT_TYPE)', () => {
    afterEach(() => {
      delete process.env.PRODUCT_TYPE;
    });

    it('skips the AsyncAPI fallback (no extra custom-content fetch) when PRODUCT_TYPE is not asyncapi', async () => {
      delete process.env.PRODUCT_TYPE; // lite / full / diagramly
      // Attachments query returns no results → the no-attachment branch.
      asAppRequest.mockResolvedValue({
        ok: true,
        status: 200,
        text: async () => '',
      ...
      const payload = {
        ...
        extensionPayload: { config: { customContentId: 'cc-lite' } },
      };

      const result = await handler(payload);

      // Only the attachments request — the custom-content fetch must NOT happen.
      expect(asAppRequest).toHaveBeenCalledTimes(1);
      expect(
        asAppRequest.mock.calls.some(([url]) => String(url).includes('/custom-content/')),
      ).toBe(false);
      // Falls through to the standard "image not yet generated" error.
      ...
      expect(
        rows.find((r) => r.event === 'macro_export_failed')?.properties?.failure_reason,
      ).toBe('attachment_not_found');
    });

    it('uses the AsyncAPI fallback (embeds the spec) when PRODUCT_TYPE === "asyncapi"', async () => {
      process.env.PRODUCT_TYPE = 'asyncapi';
      asAppRequest.mockImplementation(async (url) => {
        if (String(url).includes('/custom-content/')) {
          return {
            ok: true,
            status: 200,
            text: async () => '',
        ...
        // Attachments query → no results.
        return { ok: true, status: 200, text: async () => '', json: async () => ({ results: [] }) };
      });
      ...
      const payload = {
        ...
        extensionPayload: { config: { customContentId: 'cc-asyncapi' } },
      };

      const result = await handler(payload);

      // Both the attachments query AND the custom-content fetch ran.
      expect(asAppRequest).toHaveBeenCalledTimes(2);
      expect(
        asAppRequest.mock.calls.some(([url]) => String(url).includes('/custom-content/cc-asyncapi')),
      ).toBe(true);
      // The spec is embedded as a YAML code block.
      const json = JSON.stringify(result);
      expect(json).toContain('codeBlock');
      expect(json).toContain('asyncapi: 3.0.0');
      const rows = mixpanelBodiesFromFetch(fetch) as Array<{
        event?: string;
        properties?: { export_path?: string };
      }>;
      expect(
        rows.find((r) => r.event === 'macro_export_succeeded')?.properties?.export_path,
      ).toBe('asyncapi_spec_fallback');
    });
  });
});
